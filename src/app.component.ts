...
export class AppComponent {
  ...
  constructor() {
    // This effect listens for the isPhoneVisible signal and informs the NUI service.
    // In a real FiveM environment, this would trigger a callback to hide the cursor.
    effect(() => {
      const isVisible = this.phoneState.isPhoneVisible();
      if (!isVisible) {
   ...
   * - If screen is off, turn it on to the lock screen.
  ...
  powerButtonPress() {
    ...
    if (isVisible) {
      ...
      this.phoneState.togglePhoneVisibility(true);
    }
  }
