...
export class PhoneStateService {
  ...
  // === CORE STATE ===
  isPhoneVisible = signal(true); // Default to true for easy browser testing
  ...
  constructor() {
    this.nuiService.messages$
      .pipe(filter((message) => message.action === 'openPhone'))
      .subscribe(() => {
        this.togglePhoneVisibility(true);
      });

    // Simulate receiving an SMS after 10 seconds for demonstration
    setTimeout(() => {
        this.receiveMessage('555-0199', 'Your car is ready for pickup.');
    }, 10000);
